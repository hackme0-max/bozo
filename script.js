...
// Shorts 3D Coverflow Setup
function setupShortsCarousel() {
    const track = document.querySelector('.short-carousel-track');
    const items = document.querySelectorAll('.short-item');
    const prevBtn = document.querySelector('.short-prev');
    const nextBtn = document.querySelector('.short-next');
    ...
    function updateCarousel() {
        items.forEach((item, index) => {
            // Remove iframe if it's not the center item anymore
            const iframe = item.querySelector('iframe');
            if (iframe && index !== currentIndex) {
                iframe.remove();
                item.querySelector('.video-overlay').style.display = '';
            }

            item.classList.remove('item-center', 'item-left-1', 'item-left-2', 'item-right-1', 'item-right-2');
            
            let offset = index - currentIndex;
            const total = items.length;
            
            // Adjust offset for infinite visual loop
            if (offset < -Math.floor(total/2)) offset += total;
            if (offset > Math.floor(total/2)) offset -= total;

            if (offset === 0) item.classList.add('item-center');
            else if (offset === -1) item.classList.add('item-left-1');
            else if (offset === -2) item.classList.add('item-left-2');
            else if (offset === 1) item.classList.add('item-right-1');
            else if (offset === 2) item.classList.add('item-right-2');
        });
    }

    items.forEach((item, index) => {
        item.addEventListener('click', () => {
            // If it's the center item, play the video inline
            if (item.classList.contains('item-center')) {
                if (!item.querySelector('iframe')) {
                    const videoId = item.getAttribute('data-video-id');
                    const iframe = document.createElement('iframe');
                    iframe.src = `https://www.youtube-nocookie.com/embed/${videoId}?autoplay=1&controls=1&rel=0&modestbranding=1`;
                ...
                // If it's a side item, rotate the carousel to it
                let offset = index - currentIndex;
                const total = items.length;
                if (offset < -Math.floor(total/2)) offset += total;
                if (offset > Math.floor(total/2)) offset -= total;
...
if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', setupShortsCarousel);
} else {
    setupShortsCarousel();
}
...
function setupFeaturedCarousel() {
    ...
    function updateCarousel() {
        items.forEach((item, index) => {
            ...
            // If the item is currently the center
    ...
    // Initialize display classes based on HTML defaults
}

if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', setupFeaturedCarousel);
} else {
